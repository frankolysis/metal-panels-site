  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/categories/new" className="flex items-center gap-2 bg-brand-accent text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-orange-600"><Plus size={16} /> Add Category</Link>
      </div>
          ...
          <tbody>
            {categories.map((cat) => (
              <tr key={cat.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right"><Link href={"/admin/categories/" + cat.id + "/edit"} className="text-gray-400 hover:text-brand inline-block mr-2"><Edit size={16} /></Link><button onClick={() => handleDelete(cat.id)} className="text-gray-400 hover:text-red-600"><Trash2 size={16} /></button></td>
              </tr>
            ))}
