  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/patterns/new" className="flex items-center gap-2 bg-brand-accent text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-orange-600"><Plus size={16} /> Add Pattern</Link>
      </div>
          ...
          <tbody>
            {patterns.map((pattern) => (
              <tr key={pattern.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right"><Link href={"/admin/patterns/" + pattern.id + "/edit"} className="text-gray-400 hover:text-brand inline-block mr-2"><Edit size={16} /></Link><button onClick={() => handleDelete(pattern.id)} className="text-gray-400 hover:text-red-600"><Trash2 size={16} /></button></td>
              </tr>
            ))}
